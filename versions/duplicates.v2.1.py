...
# VERZIÓ: 2.1 (Duplikátumtörlés funkcióval kibővítve)
...
#
# 7. Duplikátumok törlése a fájlokból:
...
def collect_duplicate_lines_for_deletion(files: List[Path], duplicates_data: Dict[str, List[str]], config: Dict[str, Any], logger: logging.Logger) -> Dict[str, Dict[str, List[str]]]:
    """
    Összegyűjti a törlendő duplikált sorokat minden fájlból.
    Visszaad egy szótárat: {hash: {filename: [matching_lines]}}
    """
    logger.info("--- DUPLIKÁTUMTÖRLÉS: Törlendő sorok összegyűjtése ---")
    
    # Duplikált hash-ek létrehozása
    duplicate_prefixes = set(duplicates_data.keys())
    ...
    for file_path in tqdm(files, desc="Duplikált sorok keresése"):
        try:
            ...
            logger.error(f"Hiba a duplikált sorok keresése közben a(z) '{file_path.name}' fájlban: {e}")
    ...
    for file_path in tqdm(files, desc="Törlendő sorok részletes összegyűjtése"):
        try:
            with file_path.open('r', encoding='utf-8', errors='ignore') as f:
                header = next(f, None)
                for line in f:
            ...
            logger.error(f"Hiba a törlendő sorok részletes összegyűjtése közben a(z) '{file_path.name}' fájlban: {e}")
...
def delete_duplicate_rows(files: List[Path], lines_to_delete: Dict[str, Dict[str, List[str]]], duplicates_data: Dict[str, List[str]], config: Dict[str, Any], logger: logging.Logger) -> Dict[str, List[str]]:
    ...
    for line_hash, file_lines_map in lines_to_delete.items():
        ...
        for delete_file in delete_files:
            ...
            if delete_rows_from_file(file_path, lines_to_remove, logger):
                deleted_counts[delete_file] += len(lines_to_remove)
    
    # Kimeneti adatok frissítése: -> Deleted hozzáadása
    for prefix, file_list in modified_duplicates_data.items():
        updated_file_list = []
        for filename in file_list:
            if deleted_counts[filename] > 0:
                updated_file_list.append(f"{filename} -> Deleted")
            else:
    ...
    logger.info(f"Duplikátumtörlés befejezve. Összesen {total_deleted} sor törölve {len(deleted_counts)} fájlból.")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Párhuzamos duplikátumkereső nagy szöveges fájlokban (Verzió 2.1 - Duplikátumtörlés funkcióval).",
    ...
    parser.add_argument(
        ...
        help="Duplikált sorok törlése a fájlokból (true/false, alapértelmezett: false)."
    )
